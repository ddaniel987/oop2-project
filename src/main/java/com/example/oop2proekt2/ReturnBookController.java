...
public class ReturnBookController {
    ...
    public void initialize() throws SQLException {
        ResultSet fb = Session.getCurrentUser().lentBooks();
        while (fb.next()) {
            int id = fb.getInt("ID");
    ...
    public void onGoBtn() {
        ...
        try {
            DB.returnBook(id, Session.getCurrentUser().getId());

            Stage current = (Stage)(go.getScene().getWindow());
            current.close();

            SceneManager.Alert(Alert.AlertType.INFORMATION, "Успешно", "Вие върнахте книгата успешно.");
